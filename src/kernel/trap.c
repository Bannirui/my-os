...
/**
 * 1 CPU执行指令出错时 内部长生一个异常 并给它一个固定的向量号
 * 2 CPU的寄存器IDTR存放了IDT表的基址和界限
 * 3 检索IDT表项 表项的地址=IDT基址+异常向量号*16
 * 4 读出IDT表项上面的内容就是门描述符 读出来里面的各个字段
 * 5 IST不等于0时CPU从TSS里面取IST[n]作为新的RSP
 *   特权级变化 从TSS取RSP0换栈
 *   然后在新栈上压入现场
 * 6 跳进到异常处理函数entry.S的入口
 * 7 保存现场
 * 8 实际调用do_xxx的函数
 * 9 iretq返回
 */
