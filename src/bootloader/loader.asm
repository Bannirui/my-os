...
; 创建临时的GDT表 为了避免保护模式段结构的复杂性 把代码段和数据段的段基址都设置在0x00000000地址处 段限长0xffffffff 可以索引0到4G的内存地址空间
[SECTION gdt]
...
; 软盘加载kernel程序到内存 从哪个扇区开始找kernel.bin FAT12的根目录区是从19号扇区开始的
...
; 严格来说现在是big-real-mode模式 是有能力访问1M以上能力的
    ...
    ; 设置ds:esi=0:0x7e00 复制搬代码的src 这个时候ds会被改成0 所以等循环复制结束后要恢复ds的值
...
; 在实模式下 程序可以操作任何地址空间 而且无法限制程序的执行权限 尽管这种模式给设置硬件功能带来了许多方便 但是却给程序的执行安全和稳定都带来了隐患 一旦程序执行错误可能整个系统崩溃
; 而且实模式寻址能力有限 故而进化出保护模式
; 在保护模式里 处理器按照程序的执行级别分为0 1 2 3这4个级别 由高到低 最高级别0由系统内核使用 最低等级3由应用程序使用 Linux仅使用了0级和3级
; 保护模式不仅加入了程序的权限 还加入了分页功能 分页功能将庞大的地址空间划分为固定大小的内存页面 便于管理和缩减了应用程序之间的空间浪费现象
...
GO_TO_TMP_Protect:
    ...
    mov ax, 0x0010 ; 0x10对应的二进制是(0000 0000 00010) 000 高13位是GDT表的索引也就是2 在GDT表中所以2是数据段描述符
    ; 把所有的数据段寄存器都塞上段选择子
    mov ds, ax
    mov es, ax
    mov fs, ax
...
; 加载GDT 因为GDT表的基址和长度必须借助LGDT汇编指令才能加载到GDTR寄存器 而GDTR寄存器是1个6Byte的结构 结构中低2Byte保存GDT表的长度 高4Byte保存GDT表的基地址 GdtPtr是这个结构的起始地址
; 加载IA-32e模式的临时GDT表加载到GDTR寄存器中 并将临时GDT表的数据段初始化到各个数据段寄存器中
; 除了CS段寄存器 因为CS段寄存器不能采用直接赋值的方式来改变 所以必须借助跨段跳转指令far JMP或者跨段调用指令far CALL才能实现改变
    lgdt [GdtPtr64]
; 初始化段寄存器
    mov ax, 0x0010 ; 段选择子的偏移设置2 0x10的二进制是(0000 0000 0001 0)000 高13位是2 也就是GDT表偏移2 是数据段
    mov ds, ax
    mov es, ax
    mov fs, ax
    mov gs, ax
...
; 打开地址扩展 cr4寄存器的第5位是PAE功能的标志位 置位该标志位就开启了PAE
    ...
    bts eax, 5
    mov cr4, eax
; 在实模式->保护模式的切换过程中没有开启分页机制 就是因为考虑到32位保护模式->64位长模式切换过程中必须关闭分页机制重新构造页表结构
; 官方提供的模式切换步骤是 当页目录基址已经加载到CR3控制寄存器 接下来就可以通过置位IA32_EFER寄存器的LME标志位激活IA-32e模式
; 页目录首地址设置到crc3寄存器
...
; 使能分页
    ...
    mov cr0, eax
; 到这就完成了IA-32e 64位长模式的切换
; 但是此时处理器还是跑着32位保护模式的代码 这种状态叫做兼容模式 即运行在IA-32e 64位模式下的32位模式程序 因此需要立马执行一条调用或跨段跳转将cs寄存器的值更新成IA-32e模式的代码段描述符
; 从loader程序跳到kernel内核去 彻底切换到64位IA-32e长模式
    jmp SelectorCode64:OffsetOfKernelFile
; 检测cpu支不支持ia-32e长模式
; 由于CPUID汇编指令的扩展功能项0x80000001的第29位 指示处理器是否支持IA-32e模式 所以首先检查当前处理器对CPUID汇编指令的支持情况 判断该指令的最大扩展功能号是否超过了0x8000000 只有当CPUID指令的扩展功能号>=0x80000001时才有可能支持64位的长模式
support_long_mode:
    mov eax, 0x80000000
    cpuid
    cmp eax, 0x80000001
